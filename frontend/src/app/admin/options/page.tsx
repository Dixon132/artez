    ...
    return (
        <div className="p-8">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h2 className="text-3xl font-serif tracking-tight text-[#111]">Opciones</h2>
                    <p className="text-sm text-[#777] mt-1">{options.length} opciones</p>
                </div>
                <button
                    onClick={() => {
                        setEditingOption(null);
            ...
            <div className="space-y-4">
                {options.map((option) => (
                    <div key={option.id} className="bg-white rounded-xl border border-[#e8e4df] p-6">
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                <h3 className="text-lg font-semibold text-[#111]">{option.name}</h3>
                                <p className="text-sm text-[#777]">{option.values?.length || 0} valores</p>
                            </div>
                        ...
                        {option.values && option.values.length > 0 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                {option.values.map((value: any) => (
                                    <div key={value.id} className="border border-[#e8e4df] rounded-lg p-3 flex items-center gap-3">
                                        {value.image && (
                                            <img src={value.image} alt={value.name} className="w-12 h-12 rounded object-cover" />
                                        )}
                                        <div className="flex-1">
                                            <div className="font-medium text-[#111] text-sm">{value.name}</div>
                                            <div className="text-xs text-[#111]">+${value.base_extra_price}</div>
                                        </div>
                                        <button
